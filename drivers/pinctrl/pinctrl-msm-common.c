					...
					unsigned group)
{
	...
	for (cnt = 0; cnt < dd->pin_grps[group].num_pins; cnt++) {
		...
		pinfo->prg_func(pin, func, true, pinfo);
	}
				...
				unsigned long *configs, unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++)
		msm_pconf_prg(pctldev, pin, configs + i, true);
			...
			unsigned group, unsigned long *configs, unsigned num_configs)
{
	...
	for (cnt = 0; cnt < dd->pin_grps[group].num_pins; cnt++)
		msm_pconf_set(pctldev, pins[cnt], configs, num_configs);
